...
console.log(_showDesc);	
 let _selectedData = localStorage.getItem('dtBestPicture') ? JSON.parse(localStorage.getItem('dtBestPicture')) : [];
...
if(_showDesc){
	...
	var dt_Table = $('#dtBestPicture').DataTable( {
		"dom": "<'row'<'col-sm-9'<'toolbar'>><'col-sm-3'>><'row'<'col-sm-12'tr>>",
		...
		"createdRow": function ( row, data, index ) {
			...
			_selectedData.forEach(item => {
				if(item[0] == data[0]){_checkedState=" checked ";
				console.log(data[0]);
				}
			})
			//console.log(_selectedData[0][0]);
			
			
			var _datalinkCheck ='';			
			_datalinkCheck +='<input class="option-input checkbox" id="'+ data[0] +'"';
			_datalinkCheck += _checkedState;
			_datalinkCheck +=' type="checkbox" value="'+ data[0] +'" name="rdoBestPicture" />';			
	...
	$('#dtBestPicture tbody').on('click','input[type="checkbox"]', function (e) {
		...
		if(selectedElement.length>0)selectedIds[_pageIndex-1]=selectedElement;
		 if(_selItemCnt>1){_blockMultipleCheck(selectedElement,_pageIndex);return false;}else{localStorage.setItem('dtBestPicture', JSON.stringify(selectedElement));}
 });
