function _dtSoundMixing(_isSkiped){
	var _slPrev = 17;	
	var _slCurrent = 18;
	var _slNext = 19;		//Same for the next click function too need to update
	...
	if(_isSkiped){
		...
		$('#dtSoundDesign tbody tr').removeClass('rowSelected');
	}else{}
	
	var dt_Table = $('#dtSoundMixing').DataTable( {
		...
		_datalinkCheck +='<input class="option-input checkbox" id="'+ data[0] +'" type="checkbox" value="'+ data[0] +'" name="rdoSoundDesign" />';
		_datalinkCheck +='<label id="'+ data[0] +'"></label>';
		$('td', row).eq(0).html(_datalinkCheck);
		}
		} );
		//$('#dtSoundMixing tbody').on('click','tr', function () {
		$('#dtSoundMixing tbody').on('click','input[type="checkbox"]', function (e) {	
		...
		var _dataArr = [data[0],data[1],data[2],data[3],"Sound Mixing",data[4],data[5],data[6]];
