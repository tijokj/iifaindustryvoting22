function _dtScreenplay(_isSkiped){
	...
	var dt_Table = $('#dtScreenplay').DataTable( {
		...
		"createdRow": function ( row, data, index ) {
						...
						_datalinkCheck +='<input class="option-input checkbox" id="'+ data[0] +'" type="checkbox" value="'+ data[0] +'" name="rdoScreenplay" />';
						_datalinkCheck +='<label id="'+ data[0] +'"></label>';
						$('td', row).eq(0).html(_datalinkCheck);
		}
	} );
	
	//$('#dtScreenplay tbody').on('click','tr', function () {
	$('#dtScreenplay tbody').on('click','input[type="checkbox"]', function (e) {	
