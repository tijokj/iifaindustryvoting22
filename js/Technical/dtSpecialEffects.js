function _dtSpecialEffects(_isSkiped){
	var _slPrev = 19;	
	var _slCurrent = 20;
	var _slNext = 21;		//Same for the next click function too need to update
	...
	if(_isSkiped){
		...
		$('#dtBackgroundScore tbody tr').removeClass('rowSelected');
	}else{}
	
	var dt_Table = $('#dtSpecialEffects').DataTable( {
		...
		_datalinkCheck +='<input class="option-input checkbox" id="'+ data[0] +'" type="checkbox" value="'+ data[0] +'" name="rdoSpecialEffects" />';
		_datalinkCheck +='<label id="'+ data[0] +'"></label>';
		$('td', row).eq(0).html(_datalinkCheck);
		}
		} );
		//$('#dtSpecialEffects tbody').on('click','tr', function () {
		$('#dtSpecialEffects tbody').on('click','input[type="checkbox"]', function (e) {		
		...
		var _dataArr = [data[0],data[1],data[2],data[3],"Special Effects (Visual)",data[4],data[5],data[6]];
