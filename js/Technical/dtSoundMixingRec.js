function _dtSoundMixingRec(_isSkiped){
	/* ADDED FOR V2 */
	_showHeading(1,19);
	
	$("#Page_1_18").hide();
	$("#Page_1_19").show();
	//$("#lnkSelected").html("TECHNICAL AWARDS <i class='glyphicon glyphicon-chevron-right'></i> Re-recording Mixer");
	if(_isSkiped){
		var tempArr = ["",0,1,18,"Production Sound Mixer","","",""];
	...
	var dt_Table = $('#dtSoundMixingRec').DataTable( {
		...
		"createdRow": function ( row, data, index ) {
			...
			_datalinkCheck +='<input class="option-input checkbox" id="'+ data[0] +'" type="checkbox" value="'+ data[0] +'" name="rdoSoundMixingRec" />';
			_datalinkCheck +='<label id="'+ data[0] +'"></label>';
			$('td', row).eq(0).html(_datalinkCheck);
		}
	} );
	//$('#dtSoundMixingRec tbody').on('click','tr', function () {
	$('#dtSoundMixingRec tbody').on('click','input[type="checkbox"]', function (e) {	
		...
		$(dt_Table.$('input[type="checkbox"]:checked').map(function(){
			...
			var _dataArr = [data[0],data[1],data[2],data[3],"Re-recording Mixer",data[4],data[5],data[6]];
